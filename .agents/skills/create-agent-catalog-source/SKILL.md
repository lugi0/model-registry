...
description: >
  ...
  RHOAI model-registry. Trigger phrases include: "create agent catalog source",
...
compatible with the RHOAI model-registry catalog consumer.
...
an `agent.yaml` matching the format used by the
[agentic-starter-kits](https://github.com/red-hat-data-services/agentic-starter-kits)
repository.
...
1. **Assemble the agent.yaml content** from the extracted metadata using this exact format
   (matching the [agentic-starter-kits](https://github.com/red-hat-data-services/agentic-starter-kits)
   convention):
...
| `templates` | Use the `.agents/skills/create-agent-catalog-source/scripts/agent_yaml_to_json.py` script to convert agent.yaml to a compact, deterministic JSON string. The result becomes a single template entry: `{name: "agent.yaml", content: "<json>"}`. **For convention-discovered agents** (agent.yaml already on disk): run `python3 .agents/skills/create-agent-catalog-source/scripts/agent_yaml_to_json.py <agent-dir>/agent.yaml` and capture stdout. **For heuristic/markdown-discovered agents** (no agent.yaml on disk): write the generated agent.yaml content (confirmed by the user in Step 3c) to a temporary file (`TMPFILE=$(mktemp /tmp/agent-yaml-XXXXXX.yaml)`), run `python3 .agents/skills/create-agent-catalog-source/scripts/agent_yaml_to_json.py "$TMPFILE"`, capture stdout, then clean up (`rm "$TMPFILE"`). **Always include this field.** |
...
```bash
#!/bin/bash
# Deploy custom agent catalog source to OpenShift
...
echo "     oc edit configmap <sources-configmap-name> -n $NAMESPACE"
echo ""
